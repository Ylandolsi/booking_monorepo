...
<body>
    <div class='payment-container'>
        ...
        <div class='wallet-option' onclick='payWithWallet(""test-wallet-1"")'>
            <strong>Test Wallet 1</strong><br>
            <span class='wallet-balance'>Balance: ${(_wallets.GetValueOrDefault("test-wallet-1")?.Balance ?? 0) / 100.0:F2}</span>
        </div>
        
        <div class='wallet-option' onclick='payWithWallet(""test-wallet-2"")'>
            <strong>Test Wallet 2</strong><br>
            <span class='wallet-balance'>Balance: ${(_wallets.GetValueOrDefault("test-wallet-2")?.Balance ?? 0) / 100.0:F2}</span>
        </div>
    ...
    public async Task<IActionResult> ProcessPayment(string paymentRef, [FromBody] ProcessPaymentRequest request)
    {
        ...
        {
            case "wallet":
                if (!string.IsNullOrEmpty(request.WalletId) && _wallets.TryGetValue(request.WalletId, out var wallet))
                {
    ...
    public IActionResult ChargeWallet(string walletId, [FromBody] ChargeWalletRequest request)
    {
        ...
        if (!_wallets.TryGetValue(walletId, out var wallet))
        {
            ...
            wallet = new MockWallet { Id = walletId, Balance = 0 };
            _wallets.TryAdd(walletId, wallet);
        }
    ...
    [HttpGet("wallets/{walletId}/verify")]
    public IActionResult VerifyWallet(string walletId)
    {
        _logger.LogInformation("Mock Konnect: Verifying wallet {WalletId}", walletId);
...
{
    public string Id { get; set; } = string.Empty;
    public int Balance { get; set; } // Amount in cents
}