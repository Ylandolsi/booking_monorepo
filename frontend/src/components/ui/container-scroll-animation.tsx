...
export const ContainerScroll = ({ titleComponent, children }: { titleComponent: string | React.ReactNode; children: React.ReactNode }) => {
  ...
  const initialScale = isMobile ? 0.7 : 1.05;
  const rotate = 10;
  const translate = 0;

  return (
    <div className="relative flex h-[60rem] items-center justify-center p-2 md:h-[80rem]" ref={containerRef}>
      <div
      ...
      >
        ...
        <Card rotate={rotate} translate={translate} scale={initialScale}>
          {children}
        </Card>
      </div>
    </div>
...
export const Card = ({ rotate, scale, children }: { rotate: number; scale: number; translate: number; children: React.ReactNode }) => {
  return (
    <motion.div
      style={{
        rotateX: rotate,
