...
export function TabNavigation({ tabs, activeTab, onTabChange, className }: TabNavigationProps) {
  return (
    ...
    <div className={cn('flex flex-wrap items-center justify-center gap-2 p-1', className)}>
      {tabs.map((tab) => (
        <Button
          key={tab.id}
          variant={'outline'}
        ...
        >
          <div className="flex items-center justify-center space-x-2">
            ...
            <div>
              ...
              {tab.description && <div className={cn('mt-0.5 text-xs', activeTab === tab.id ? '' : 'text-muted-foreground')}>{tab.description}</div>}
            </div>
          </div>
        </Button>
      ))}
