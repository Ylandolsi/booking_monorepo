...
const ROUTE_PATHS = {
  ...
  APP: {
    ...
    STORE: {
      INDEX: "/app/store/",
      // show all products and store preview mobile with add product button
      PRODUCT: {
        VIEW: "/app/store/product/$productSlug",
        // view product details page
        // type : ?type=booking|digital
        INDEX: "/app/store/product"
        // add product flow : select type -> details -> specific fields
      },
      INFO_EDIT: "/app/store/edit",
      // store header info edit page
      CREATE_STORE: "/app/store/create",
      // create store flow
      SETUP_STORE: "/app/store/setup",
      // initial setup wizard after creating store
...
const routeBuilder = {
  ...
  auth: {
    ...
    emailVerificationVerified: (params) => {
      const query = params?.redirectTo ? `?redirectTo=${encodeURIComponent(params.redirectTo)}` : "";
  ...
  store: {
    ...
    productView: ({ productSlug }) => ROUTE_PATHS.APP.STORE.PRODUCT.VIEW.replace("$productSlug", productSlug),
    productAdd: (params) => {
    ...
    // public store preview
    publicStorePreview: ({ storeSlug }) => ROUTE_PATHS.APP.STORE.PUBLIC_PREVIEW.replace("$storeSlug", storeSlug),
    publicSessionProduct: ({ storeSlug, productSlug }) => ROUTE_PATHS.APP.STORE.PUBLIC_SESSION_PRODUCT.replace("$storeSlug", storeSlug).replace("$productSlug", productSlug)
  },
  // Booking routes
  booking: {
    session: (mentorSlug) => ROUTE_PATHS.APP.BOOKING.SESSION.replace(ROUTE_PARAMS.MENTOR_SLUG, mentorSlug)
  },
  meets: {
  ...
  admin: {
    ...
    payoutRequestDetails: (requestId) => ROUTE_PATHS.APP.ADMIN.PAYOUT_REQUEST_DETAILS.replace(ROUTE_PARAMS.REQUEST_ID, requestId)
  },
  // Error/Test routes
  errorExp: {
...
const routes = {
  ...
  isAppRoute: (path) => path.startsWith("/app"),
  isBookingRoute: (path) => path.startsWith("/booking"),
  isMentorRoute: (path) => path.startsWith("/mentor"),
  isProfileRoute: (path) => path.startsWith("/profile"),
  isTestRoute: (path) => path.startsWith("/test") || path.startsWith("/error-exp")
